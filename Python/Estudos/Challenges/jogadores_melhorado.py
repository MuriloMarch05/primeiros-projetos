...
melhor_partida = dados['gols'].index(max(dados['gols'])) + 1    # Índice da melhor partida
pior_partida = dados['gols'].index(min(dados['gols'])) + 1  # Índice da pior partida

print('-=' * 30)
print('RESUMO DO JOGADOR'.center(60))
print('-=' * 30)
print(f'NOME: {dados["nome"]}\nPARTIDAS: {partidas}\nTOTAL DE GOLS: {dados["total"]}\nMÉDIA DE GOLS POR PARTIDA: {media:.1f}')
print(f'MELHOR PARTIDA: Partida {melhor_partida} com {max(dados["gols"])} gols')
print(f'PIOR PARTIDA: Partida {pior_partida} com {min(dados["gols"])} gols.')
